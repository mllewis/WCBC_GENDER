...
\keywords{reading, gender, language development\newline\indent Word count: 2060 (excluding methods and results but INCLUDING footnotes)}
\usepackage{csquotes}
\usepackage{setspace}
\usepackage{float}
\usepackage{graphicx}
...
As a first step we asked adult English speakers to rate the genderedness of words in these books using a 5-point scale from masculine to feminine (Scott, Keitel, Becirspahic, Yao, \& Sereno, 2019)\footnote{We operationalize gender as a continuum ranging from masculine to feminine throughout, and use the terms "masculine" and "feminine" interchangebly with "male" and "female". This approach admittidely ignores many aspects of gender that not critical to the present research question.} This procedure yields systematic data with good face validity: words such as \enquote{axe} and \enquote{engine} as masculine, \enquote{cuddle} and \enquote{pink} are rated as feminine,\enquote{exactly} and \enquote{nose} as neutral.
