...
\keywords{reading, gender, language development\newline\indent Word count: 1972 (excluding methods and results)}
\usepackage{csquotes}
\usepackage{upgreek}
...
\hypertarget{study-1c-validation-of-human-judgments-as-estimates-of-book-bias}{%
\subsection{Study 1c: Validation of human judgments as estimates of book bias}\label{study-1c-validation-of-human-judgments-as-estimates-of-book-bias}}
...
Consistent with this general pattern, books with female primary characters also tended to be more often read to girls, compared to the overall average (\emph{t}(46) = 7.02, \emph{p} \textless{} .001; \emph{d} = 1.02 {[}0.66, 1.6{]}; \autoref{fig:characterplots}b). Books with male (\emph{t}(92) = -5.12, \emph{p} \textless{} .001; \emph{d} = -0.53 {[}-0.71, -0.35{]}) or gender indeterminate primary characters (\emph{t}(68) = -3.24, \emph{p} = 0.002; \emph{d} = -0.39 {[}-0.59, -0.18{]}) tended to be more often read to boys. Notably, the effect size for girls was more than twice that of boys, suggesting that there was a stronger bias to read books with female content to girls, relative to books with male content to boys. There was no bias in audience gender for books with multiple primary characters of different genders (\emph{t}(16) = 0.24, \emph{p} = 0.81; \emph{d} = 0.06 {[}-0.37, 0.77{]}) or books without primary characters (\emph{t}(19) = -0.56, \emph{p} = 0.58; \emph{d} = -0.13 {[}-0.74, 0.31{]}).
